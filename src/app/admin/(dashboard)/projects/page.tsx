    ...
    <div className="space-y-6">
      ...
      <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {projects.map((project) => (
          <li key={project.id} className="flex flex-col rounded-xl border border-slate-200 bg-white p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-brand-700">{project.category}</p>
                <h2 className="mt-1 text-base font-bold text-slate-900">{project.name}</h2>
              </div>
              {project.featured ? (
            ...
            <p className="mt-2 flex-1 text-sm text-slate-600">{truncate(project.description, 110)}</p>
            <div className="mt-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <StatusBadge status={project.isPublished ? "published" : "unpublished"} />
              ...
              <div className="flex items-center gap-1">
                ...
                <DeleteButton id={project.id} action={deleteProject} confirmMessage="Delete this project permanently?" />
              </div>
            </div>
          </li>
        ))}
